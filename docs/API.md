...
- `TRY_ON_API_URL` — базовый endpoint AI-провайдера 2D/3D примерки.
- `TRY_ON_API_TOKEN` — токен авторизации запросов к провайдеру.
...
- `400 Bad Request` — неподдерживаемый источник товара или редирект на иной домен (поддерживаются только HTTPS-ссылки wildberries.ru, ozon.ru или lamoda.ru).
...
- `503/500` — сервис 2D-примерки не настроен (проверьте env `TRY_ON_API_URL`/`TRY_ON_API_TOKEN`).
...
- `503/500` — сервис 3D-примерки не настроен (проверьте env `TRY_ON_3D_API_URL`).
