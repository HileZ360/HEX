...
## Требования
- Node.js 20.19+ (Node 18 вызывает ошибки запуска Vite/undici)
- npm 10+ (фиксировано в `engines` корневого `package.json`)
- Рекомендуется использовать `npm@10.x` (см. поле `packageManager`)

## Установка и запуск
Все команды ниже предполагают установленный Node.js 20+.
